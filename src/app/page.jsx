...
/* ════════════════════════════════════════════════════
   BANNER CHIMOLA — sección en el inicio
════════════════════════════════════════════════════ */
function ChimolaBanner() {
  const { dispatch } = useStore();
  const waUrl = `https://wa.me/${WA_NUM}?text=${encodeURIComponent('Hola MaxFarma, quisiera consultar precios mayoristas de la marca CHIMOLA.')}`;

  return (
    <div className="max-w-7xl mx-auto px-4 mt-14">
      <div className="relative overflow-hidden rounded-2xl" style={{ background:'linear-gradient(135deg,#1a1a1a 0%,#2d1f0e 50%,#1a1a1a 100%)' }}>
        {/* Fondo decorativo */}
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-center gap-6 px-8 py-10">
          {/* Texto */}
          <div className="flex-1 text-center sm:text-left">
            <span className="inline-flex items-center gap-1.5 bg-amber-500/20 border border-amber-500/30 text-amber-400 text-xs font-bold px-3 py-1 rounded-full mb-3">
              <span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse" />
              Marca exclusiva
            </span>
            <h2 className="text-white text-2xl sm:text-3xl font-black tracking-tight mb-1">CHIMOLA</h2>
            <p className="text-amber-200/70 text-sm mb-2">Carteras · Billeteras · Mochilas · Accesorios</p>
            <p className="text-gray-400 text-xs mb-5">
              Consultá precios mayoristas para revendedores. Asesoramiento sin compromiso.
            </p>
            <div className="flex flex-wrap gap-3 justify-center sm:justify-start">
              <button
                onClick={() => dispatch({ type:'SET_SECTION', payload:'chimola' })}
                className="inline-flex items-center gap-2 bg-amber-500 hover:bg-amber-400 text-black font-bold px-5 py-2.5 rounded-xl transition-colors text-sm shadow-lg">
                Ver catálogo CHIMOLA
              </button>
              <a href={waUrl} target="_blank" rel="noopener noreferrer"
                className="inline-flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white font-semibold px-5 py-2.5 rounded-xl transition-colors text-sm border border-white/20">
...
/* ════════════════════════════════════════════════════
   BANNER RECETAS — editable desde panel Admin → Contenido
════════════════════════════════════════════════════ */
...
function RecetaBanner() {
  const { state } = useStore();
  const cfg = { ...RECETA_DEFAULTS, ...(state.contenido?.receta_banner || {}) };

  if (!cfg.activo) return null;

  const waUrl = `https://wa.me/5493625298918?text=${encodeURIComponent('Hola MaxFarma, quiero enviar una receta médica para que preparen mi medicamento.')}`;

  return (
    <div className="max-w-7xl mx-auto px-4 mt-8">
      <div className="relative overflow-hidden rounded-2xl" style={{ background: cfg.color_fondo }}>
        {/* Fondo imagen si tiene */}
        {cfg.imagen_url && (
          <img src={cfg.imagen_url} alt=""
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-center gap-6 px-6 sm:px-10 py-8">
          {/* Ícono */}
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center flex-shrink-0 border border-white/20"
            style={{ background: cfg.color_acento + '30' }}>
            <svg className="w-8 h-8" style={{ color: cfg.color_acento }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          ...
          {/* Texto */}
          <div className="flex-1 text-center sm:text-left">
            <p className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: cfg.color_acento }}>
              Servicio de recetas
            </p>
            <h2 className="text-white text-xl sm:text-2xl font-black tracking-tight mb-1">
              {cfg.titulo}
            </h2>
          ...
          <div className="flex-shrink-0">
            <a href={waUrl} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-2.5 font-bold px-6 py-3.5 rounded-xl transition-colors shadow-lg text-sm text-white whitespace-nowrap"
