...
export default function Header() {
  ...
  const pickSuggestion = (p) => {
    setQuery(p.nombre);
    dispatch({ type: 'SET_SEARCH', payload: p.nombre });
    ...
    <>
      ...
      <header className={`sticky top-0 z-50 border-b border-gray-100 transition-all duration-200 ${scrolled ? 'navbar-scroll shadow-md' : 'bg-white'}`}>
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center gap-3">
          ...
          {/* Logo — h-11 (más grande que antes h-9) */}
                ...
                {suggestions.length > 0 ? (
                  ...
                  <>
                    ...
                    {state.searchHistory.map((h, i) => (
                      <button key={i} onClick={() => { setQuery(h); dispatch({ type: 'SET_SEARCH', payload: h }); setSearchFocused(false); }}
        ...
        <div className="sm:hidden px-4 pb-3">
          <div className="relative">
            ...
            <input type="text" placeholder="Buscá productos..." value={query} onChange={e => handleSearch(e.target.value)}
